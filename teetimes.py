...
def report():
    ...
    if leads:
        cutoff = leads[max(0, int(len(leads) * 0.10) - 1)]
    ...
    tol = interval * 1.5  # can't time a vanish tighter than our sampling rate
    ...
    for play_date in sorted({d for d, _ in hist}):
        for tee_dt in _fringe_slots(play_date):
            ...
            if h is None:
                # on the sheet, never once seen open -> gone before we started
                unseen.append((key, {"tee_dt": tee_dt, "first_spots": None,
    ...
    dump("BOOKED (vanished well before cutoff)", booked)
    dump("BOOKED BEFORE TRACKING STARTED (never seen open)", unseen)
